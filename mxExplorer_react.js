...
mxExplorer.modalContentMaxInitialHeight = mxExplorer.maxInitialHeight - mxExplorer.navPanelHeight - (2 * mxExplorer.resizeAreaSize);
...
mxExplorer.styleTextNode = document.createTextNode(
	...
	+ ".mxExplorerModal .cell .hidden {display: none;}"

	+ ".mxExplorerModal a {color: blue;"
	+ "cursor: pointer;}"

	+ ".mxExplorerModal .ns-resize {cursor: ns-resize;}"
	+ ".mxExplorerModal .ew-resize {cursor: ew-resize;}"

	+ ".mxExplorerModal .nesw-resize {cursor: nesw-resize;}"
	+ ".mxExplorerModal .nwse-resize {cursor: nwse-resize;}"
	...
	+ ".mxExplorerModal .searchBarCell {"
	+ "padding: 5px 20px;}"

	+ ".mxExplorerModal .cell .overflow {"
	+ "overflow: auto;"
	+ "display: block}"
...
function addBrowseEntitiesModel() {
	mxExplorer.browseEntitiesModel = addModalClosableDraggable(mxExplorer.body, "Browse entities");

	const entitiesContainer = addContentContainer(mxExplorer.browseEntitiesModel.contentCell);
	const contentTable = addTable(entitiesContainer);
	const searchRow = addRow(contentTable);
	const searchRowCell = addCell(searchRow);
	addClass(searchRowCell, 'searchBarCell');
	...
	addClickableButton(searchRowTableResetSearchButtonCell, "Clear", () => {
		searchField.value = "";
		searchEntities("");
	});

	const entityRow = addRow(contentTable);
	const entityCell = addCell(entityRow);
	addClass(entityCell, "overflow");

	addEntities(addTable(entityCell));
	initModal(mxExplorer.browseEntitiesModel);
	entityCell.style.height = (mxExplorer.browseEntitiesModel.offsetHeight - (mxExplorer.navPanelHeight + 85)) + "px";
}

function addBrowseCacheModel() {
	mxExplorer.browseCacheModel = addModalClosableDraggable(mxExplorer.body, "Browse cache");
	...
	addClickableButton(refreshButtonCell, "Refresh", () => {
...
function addEntities(entitiesTable) {
	...
	let entityCounter = 1;
	mxExplorer.entityKeys.forEach(function (item) {
...
function addCacheContainerContent(contentTable) {
	let entityCounter = 1;
	window.mx.data.getObjectsStatistics().forEach(function (item) {
...
function addConstantsTableContent(contentTable) {
	let entityCounter = 1;
	mx.session.getConfig().constants.forEach(function (item) {
...
function addEntityAttributePanel(attributeColumn, entityName, attributes) {
	...
	attributes.forEach((attribute) => {
		...
		addTextNode(column, attribute + " (" + entityObject.getAttributeType(attribute) + ")");
		if (writeAccess) {
...
function addLabelValue(table, labelParam, valueParam) {
	...
	return addLabelObject(table, labelParam, valueContainer);
}
...
function addResizableModal(parent, title) {
	...
	addResizeContainer(topleft, "nwse", true, true, modal, "tl");

	const topCenter = addCell(topRow);
	addClass(topCenter, "fitContent");
	addResizeContainer(topCenter, "ns", true, false, modal, "t");

	const topRight = addCell(topRow);
	addClass(topRight, "fitContent");
	addResizeContainer(topRight, "nesw", true, true, modal, "tr");

	//Center row
	const centerRow = addRow(modalTable);
	const centerleft = addCell(centerRow);
	addClass(centerleft, "fitContent");
	const centerLeftResizeContainer = addResizeContainer(centerleft, "ew", false, true, modal, "l");
	...
	const centerRightResizeContainer = addResizeContainer(centerRight, "ew", false, true, modal, "r");
	...
	addResizeContainer(bottomleft, "nesw", true, true, modal, "bl");

	const bottomCenter = addCell(bottomRow);
	addClass(bottomCenter, "fitContent");
	addResizeContainer(bottomCenter, "ns", true, false, modal, "b");

	const bottomRight = addCell(bottomRow);
	addClass(bottomRight, "fitContent");
	addResizeContainer(bottomRight, "nwse", true, true, modal, "br");
...
function addDataGrid(parent, parentAttributes, entity, hiddenXpathValue, embedded, modal) {
	...
	dataGrid.pageOffset = 0;
	dataGrid.offset = 0;
	dataGrid.totalPages = 0;
	dataGrid.totalResultSize = 0;
	dataGrid.resultSize = 0;
	dataGrid.parentAttributes = parentAttributes;
	dataGrid.modal = modal;
	dataGrid.xpath = "";
	dataGrid.isFirstPage = true;
	dataGrid.isLastPage = true;
	dataGrid.sortSpec = [];
	dataGrid.hiddenColumns = new Set();
	dataGrid.currentDataCells = [];

	addDataGridHeader(dataGrid);
	addDataGridContent(dataGrid);
	addDataGridNavigation(dataGrid);
	updateDataGrid(dataGrid);
}

function refreshSortingContainer(dataGrid) {
	const sortingContainer = dataGrid.sortingContainer;
	while (sortingContainer.hasChildNodes()) {
		sortingContainer.removeChild(sortingContainer.lastChild);
	}
	let count = 0;
	if (dataGrid.sortSpec && dataGrid.sortSpec.length > 0) {
		if (dataGrid.sortSpec.length === 1) {
			addClass(sortingContainer, "singleSortingAttribute");
		} else {
			addDropTarget(dataGrid, sortingContainer, count);
		}

		dataGrid.sortSpec.forEach((sort) => {
...
function updateDataGrid(dataGrid) {
	...
	const parentAttributes = dataGrid.parentAttributes;

	mxDataGet("//" + dataGrid.entity + dataGrid.hiddenXpathValue + dataGrid.xpath,
		(entries, countObject) => {

			dataGrid.totalResultSize = countObject.count;
			...
			entries.forEach(entry => {
				...
				parentAttributes.forEach(attribute => {
					let value = getValue(entry, attribute, dataGrid.entityObject);
					const dataCell = addCell(dataRow);
					addClass(dataCell, "dataCell");
					rowCells.set(attribute, dataCell);
				...
				dataGrid.currentDataCells.push(rowCells);
			});
			dataGrid.resultSize = entries.length;
			dataGrid.totalPages = Math.ceil(dataGrid.totalResultSize / dataGrid.pageSize);
...
function addDataGridHeader(dataGrid) {
	...
	const xpathField = addTextArea(topCenterColumnLabelCell, 100);
	dataGrid.xpathField = xpathField;
	xpathField.placeholder = "Type your xpath here, don't forget the []";

	xpathField.addEventListener("keydown", (event) => {
...
function addDataGridContent(dataGrid) {
	...
	const dataTable = addTable(dataGridContent);
	...
	guidCell.addEventListener("click", () => {
		addTextToInputField(dataGrid.xpathField, "[id = ]");
	});
...
function addDataGridNavigation(dataGrid) {
	const navigationRow = addRow(dataGrid);
	dataGrid.navigationRow = navigationRow;
	...
	dataGrid.isFirstPage = true;
	dataGrid.isLastPage = false;

	addPaging(dataGrid);
	dataGrid.firstPageLink = addLink(firstPageColumn, "<<");
	dataGrid.previousPageLink = addLink(previousPageColumn, "<");
	...
	//Don't handle click if we're already on the first page
	dataGrid.firstPageLink.addEventListener("click", () => {
		if (!dataGrid.isFirstPage) {
			handlePageNavigationClick(dataGrid, 0);
		}
	});

	//Don't handle click if we're already on the first page
	dataGrid.previousPageLink.addEventListener("click", () => {
		if (!dataGrid.isFirstPage) {
			handlePageNavigationClick(dataGrid, dataGrid.pageOffset - 1);
		}
	});

	//Don't handle click if we're already on the last page
	dataGrid.nextPageLink.addEventListener("click", () => {
		if (!dataGrid.isLastPage)  {
			handlePageNavigationClick(dataGrid, dataGrid.pageOffset + 1);
		}
	});

	//Don't handle click if we're already on the last page
	dataGrid.lastPageLink.addEventListener("click", () => {
		if (!dataGrid.isLastPage) {
			handlePageNavigationClick(dataGrid, dataGrid.totalPages - 1);
		}
	});
}

function handlePageNavigationClick(dataGrid, newPageOffset) {
	//Make sure we don't request any illegal pages
	if (!(newPageOffset < 0) && !(newPageOffset > dataGrid.totalPages - 1)) {
		dataGrid.pageOffset = newPageOffset;
		dataGrid.isFirstPage = dataGrid.pageOffset <= 0;
		dataGrid.isLastPage = dataGrid.pageOffset >= dataGrid.totalPages - 1;
...
function getValue(entry, attribute) {
...
function addAssociationPanel(association, entity, table, evenRow, modal, entry) {
	...
	addCollapsiblePanel(container, (headerCell) => {
		...
		if (npEntity) {
			...
			window.mx.data.getObjectsStatistics().forEach((item) => {
				...
				if (entityObject.getEntity() === entity) {
					if (getValue(entityObject, key, mxExplorer.entities[entityObject.getEntity()]) === entry.getGuid()) {
						...
						link.addEventListener("click", (event) => {
							...
							window.mx.data.getObjectsStatistics().every((item) => {
...
function updatePaging(dataGrid) {
	const label = dataGrid.pagingLabel;
	if (dataGrid.resultSize > 0) {
		const maxLength = dataGrid.offset + dataGrid.pageSize;
		const toString = dataGrid.resultSize < maxLength ? dataGrid.offset + dataGrid.resultSize : maxLength;
...
function initModal(modal, topLeft = false) {
	...
	if (modal.dataGridContent !== undefined && modal.dataGridContent.offsetHeight > mxExplorer.modalContentMaxInitialHeight) {
		setComponentHeightInPixels(modal.dataGridContent, mxExplorer.dataGridContentMaxInitialHeight);
	}
...
function loadAssociations() {
	mxExplorer.entityKeys.forEach((entityKey) => {
		const entityObject = mxExplorer.entities[entityKey];
		entityObject.getReferenceAttributes().forEach((reference) => {
			let referenceNotFromSuper = true;
			//Check reference for presence in super class
			entityObject.getSuperEntities().every((superEntity) => { // Changed from getSuperEntities(entity)
			...
			if (referenceNotFromSuper) {
				const associatedEntityKey = entityObject.getSelectorEntity(reference);
...
function addResizeContainer(parent, cursorType, setHeight, setWidth, modal, location) {
	...
	switch(cursorType) {
		case "ns":
			addClass(container, "ns-resize");
			break;
		case "ew":
			addClass(container, "ew-resize");
			break;
		case "nesw":
			addClass(container, "nesw-resize");
			break;
		case "nwse":
			addClass(container, "nwse-resize");
			break;
		default:
		// code block
	}
	...
	container.addEventListener("dragend", (event) => {
		...
		if (modal.dataGridContent !== undefined) {
			if (widthDifMod != null) {
				modal.dataGridContent.style.width = modal.dataGridContent.offsetWidth + widthDifMod;
...
function addColumnPickerCell(dataGrid, headerRow) {
	const eyeCell = addCell(headerRow);
	addClass(eyeCell, "columnPickerCell");

	const wrapper = addContainer(eyeCell);
	addClass(wrapper, "columnPickerWrapper");

	const eyeButton = addMiniButton(wrapper, "👁");
	eyeButton.title = "Show/hide columns";

	eyeButton.addEventListener("click", (event) => {
		event.stopPropagation();
		if (dataGrid.columnPickerDropdown) {
			dataGrid.columnPickerDropdown.remove();
			dataGrid.columnPickerDropdown = null;
		} else {
			showColumnPickerDropdown(dataGrid, wrapper);
		}
	});
}

function showColumnPickerDropdown(dataGrid, wrapper) {
	const dropdown = addContainer(wrapper);
	addClass(dropdown, "columnPickerDropdown");
	dataGrid.columnPickerDropdown = dropdown;

	// --- Select/deselect all row ---
	const selectAllItem = addContainer(dropdown);
	addClass(selectAllItem, "columnPickerItem");

	const selectAllCheckbox = document.createElement("input");
	selectAllCheckbox.type = "checkbox";

	const selectAllLabel = document.createElement("span");
	addTextNode(selectAllLabel, "Select/deselect all");
	...
	addClass(divider, "columnPickerDivider");
	dropdown.appendChild(divider);

	// --- Column rows ---
	const columnCheckboxes = [];

	dataGrid.parentAttributes.forEach(attribute => {
		const item = addContainer(dropdown);
		addClass(item, "columnPickerItem");

		const checkbox = document.createElement("input");
		checkbox.type = "checkbox";
		checkbox.checked = !dataGrid.hiddenColumns.has(attribute);
		columnCheckboxes.push(checkbox);

		const label = document.createElement("span");
		addTextNode(label, attribute);

		item.appendChild(label);
		item.appendChild(checkbox);

		checkbox.addEventListener("change", () => {
			toggleColumn(dataGrid, attribute, checkbox.checked);
			updateSelectAllState(selectAllCheckbox, columnCheckboxes);
		});
	...
	selectAllCheckbox.addEventListener("change", () => {
		const makeVisible = selectAllCheckbox.checked;
		selectAllCheckbox.indeterminate = false;
		columnCheckboxes.forEach((cb, i) => {
			cb.checked = makeVisible;
			toggleColumn(dataGrid, dataGrid.parentAttributes[i], makeVisible);
		});
	});
...
function updateSelectAllState(selectAllCheckbox, columnCheckboxes) {
	const checkedCount = columnCheckboxes.filter(cb => cb.checked).length;
...
function toggleColumn(dataGrid, attribute, visible) {
	const headerCell = dataGrid.headerCellMap.get(attribute);
	if (visible) {
		dataGrid.hiddenColumns.delete(attribute);
		removeHiddenClass(headerCell);
		dataGrid.currentDataCells.forEach(rowCells => {
			if (rowCells.has(attribute)) {
				removeHiddenClass(rowCells.get(attribute));
			}
		});
	} else {
		dataGrid.hiddenColumns.add(attribute);
		addHiddenClass(headerCell);
		dataGrid.currentDataCells.forEach(rowCells => {
			if (rowCells.has(attribute)) {
				addHiddenClass(rowCells.get(attribute));
			}
		});
	}
}
