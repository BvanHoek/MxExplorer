...
function addDataGrid(parent, parentAttributes, entity, hiddenXpathValue, embedded, modal) {
	...
	dataGrid.entityObject = mxExplorer.entities.get(this.entity);
		...
		(entries, countObject) => {
			...
			entries.forEach(entry => {
				...
				parentAttributes.forEach(attribute => {
					const value = getValue(entry, attribute, dataGrid.entityObject);
					...
					const element = addValueElement(dataCell, value, attribute, entry.metaData);
...
function addDataGridContent(dataGrid) {
	...
	const entityObject = mxExplorer.entities.get(dataGrid.entity);
		...
		if (npEntity) {
			...
			window.mx.data.getObjectsStatistics().forEach((item, index) => {
				...
				if (entityObject.getEntity() === entity) {
					if (getValue(entityObject, key, mxExplorer.entities.get(entityObject.getEntity())) === entry.getGuid()) {
						hasNpObjects = true;
...
function addColumnPickerCell(dataGrid, headerRow) {
	...
	eyeButton.addEventListener("click", (event) => {
		...
		if (dataGrid.columnPickerDropdown) {
			...
			showColumnPickerDropdown(dataGrid, wrapper, eyeButton);
		}
	});
}

function showColumnPickerDropdown(dataGrid, wrapper, eyeButton) {
	...
	const closeDropdown = (event) => {
		if (dataGrid.columnPickerDropdown &&
			!dataGrid.columnPickerDropdown.contains(event.target) &&
			event.target !== eyeButton) {
			dataGrid.columnPickerDropdown.remove();
			dataGrid.columnPickerDropdown = null;
